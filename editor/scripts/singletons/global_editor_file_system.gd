...
func _check_external_changes_manual() -> void:
	if root_path.is_empty() || _is_logic_processing:
		return
	...
	while file_name != "":
		if file_name != "." and file_name != "..":
			...
			if _resource_cache.has(full_path):
				...
				_resource_cache[full_path] = {"is_dir": is_dir, "verified": true}
			if is_dir:
...
func execute_move_batch_undoable(
	paths: Array[String], dest_dir: String, action_name: String = "移动项目"
) -> void:
	if paths.is_empty():
		return
...
func execute_paste_batch_copy(clipboard_paths: Array[String], dest_dir: String) -> void:
	for src in clipboard_paths:
		if dest_dir == src || dest_dir.begins_with(src + "/"):
			continue
...
func _copy_recursive(from: String, to: String) -> void:
	if DirAccess.dir_exists_absolute(from):
		...
		if dir:
			...
			while fn != "":
				if fn != "." && fn != "..":
					_copy_recursive(from.path_join(fn), to.path_join(fn))
				fn = dir.get_next()
	else:
