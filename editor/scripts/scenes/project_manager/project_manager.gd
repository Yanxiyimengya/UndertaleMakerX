...
func sort_project_item_list() -> void:
	...
	items.sort_custom(
		func(a: ProjectListItem, b: ProjectListItem):
...
func delete_project_item(item: ProjectListItem) -> void:
	...
	WindowManager.open_confirmation_window(
		tr("Remove Project"),
		message,
		func(confirm: bool):
			if confirm:
...
func _on_recent_button_pressed() -> void:
	sort_order = (
		SortOrder.ASCENDING if (sort_order == SortOrder.DESCENDING) else SortOrder.DESCENDING
	)
	sort_project_item_list()
...
func _on_create_button_pressed() -> void:
	WindowManager.open_create_project_window(
		_on_create_project_window_content_create_project_requset
	)
