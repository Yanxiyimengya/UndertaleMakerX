...
public partial class GameoverScene : Node
{
    ...
    private async void Play()
    {
        ...
        UtmxGlobalStreamPlayer.Instance.PlaySound(UtmxGlobalStreamPlayer.Instance.GetStreamFormLibrary("HEART_BEAT_BREAK"));
        await ToSignal(GetTree().CreateTimer(1.5), Timer.SignalName.Timeout);

        SoulSprite2D.Visible = false;
        UtmxGlobalStreamPlayer.Instance.PlaySound(UtmxGlobalStreamPlayer.Instance.GetStreamFormLibrary("HEART_PLOSION"));
        for (int i = 0; i < 4; i++)
        {
        ...
        UtmxGlobalStreamPlayer.Instance.PlayBGM("GAME_OVER", UtmxGlobalStreamPlayer.Instance.GetStreamFormLibrary("GAME_OVER"));
    ...
    {
        UtmxGlobalStreamPlayer.Instance.SetBgmVolume("GAME_OVER", -72, true, 2.0);
        _inputAcceptable = false;
        GameoverTextTyper.Visible = false;
        Tween _tween = CreateTween();
