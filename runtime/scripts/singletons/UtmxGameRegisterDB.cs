...
partial class EncounterRegisterData : BaseEncounterRegisterData
{
    ...
    public EncounterRegisterData(Type encounterType)
    {
        ...
        if (encounterType.IsAbstract || encounterType.IsInterface)
            UtmxLogger.Error($"{encounterType.Name} {TranslationServer.Translate("is an abstract class or interface and cannot be instantiated.")}");

        if (!typeof(BaseEncounter).IsAssignableFrom(encounterType))
            UtmxLogger.Error($"{encounterType.Name} {TranslationServer.Translate("Cannot register because it is not a subclass of the specified type")}");
    ...
    public override BaseEncounter GetInstance()
    {
        ...
        {
            UtmxLogger.Error($"{TranslationServer.Translate("Failed to instantiate object")} {_encounterType.Name}: {TranslationServer.Translate("Missing public constructor")}: {ex.Message}");
        }
        catch (Exception ex)
        {
            UtmxLogger.Error($"{TranslationServer.Translate("Failed to instantiate object")} {_encounterType.Name}: {ex.Message}");
        }
        return null;
    }
}
partial class EnemyRegisterData : BaseEnemyRegisterData
{
    ...
    public EnemyRegisterData(Type enemyType)
    {
        ...
        if (enemyType.IsAbstract || enemyType.IsInterface)
            UtmxLogger.Error($"{enemyType.Name} {TranslationServer.Translate("is an abstract class or interface and cannot be instantiated.")}");

        if (!typeof(BaseEnemy).IsAssignableFrom(enemyType))
            UtmxLogger.Error($"{enemyType.Name} {TranslationServer.Translate("Cannot register because it is not a subclass of the specified type")}");
    ...
    public override BaseEnemy GetInstance()
    {
        ...
        {
            UtmxLogger.Error($"{TranslationServer.Translate("Failed to instantiate object")} {_enemyType.Name}: {TranslationServer.Translate("Missing public constructor")}: {ex.Message}");
        }
        catch (Exception ex)
        {
            UtmxLogger.Error($"{TranslationServer.Translate("Failed to instantiate object")} {_enemyType.Name}: {ex.Message}");
        }
        return null;
    }
}
partial class ItemRegisterData : BaseItemRegisterData
{
    ...
    public ItemRegisterData(Type itemType)
    {
        ...
        if (itemType.IsAbstract || itemType.IsInterface)
            UtmxLogger.Error($"{itemType.Name} {TranslationServer.Translate("is an abstract class or interface and cannot be instantiated.")}");

        if (!typeof(BaseItem).IsAssignableFrom(itemType))
            UtmxLogger.Error($"{itemType.Name} {TranslationServer.Translate("Cannot register because it is not a subclass of the specified type")}");
        _itemType = itemType;
    }
    public override BaseItem GetInstance()
    {
        ...
        {
            UtmxLogger.Error($"{TranslationServer.Translate("Failed to instantiate item object")} {_itemType.Name}: {TranslationServer.Translate("Missing public constructor")}: {ex.Message}");
        }
        catch (Exception ex)
        {
            UtmxLogger.Error($"{TranslationServer.Translate("Failed to instantiate object")} {_itemType.Name}: {ex.Message}");
        }
        return null;
    }
}
...
public partial class UtmxGameRegisterDB
{
    ...
    public static bool TryGetEncounter(string encounterId, out BaseEncounter encounter)
    {
        ...
        {
            UtmxLogger.Error($"{TranslationServer.Translate("Invalid")} '{encounterId}' {TranslationServer.Translate("Please check if the object has completed registration.")}");
        }
        return false;
    }
    ...
    public static bool TryGetEnemy(string enemyId, out BaseEnemy enemy)
    {
        ...
        {
            UtmxLogger.Error($"{TranslationServer.Translate("Invalid")} '{enemyId}' {TranslationServer.Translate("Please check if the object has completed registration.")}");
        }
        return false;
    }
    ...
    public static bool TryGetItem(string itemId, out BaseItem item)
    {
        ...
        {
            UtmxLogger.Error($"{TranslationServer.Translate("Invalid")} '{itemId}' {TranslationServer.Translate("Please check if the object has completed registration.")}");
        }
        return false;
    }
