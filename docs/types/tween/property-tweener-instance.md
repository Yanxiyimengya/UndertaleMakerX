...
```javascript
delay(delayTime: number) -> PropertyTweenerInstance
```
...
设置当前补间完成时执行的回调函数，该回调函数不接收任何参数和返回值。
...
```javascript
relative() -> PropertyTweenerInstance
```

将补间目标值设置为相对值（基于起始值的偏移量），而非绝对值。
